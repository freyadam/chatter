...
static void poll_cycle(struct pollfd ** fds_ptr, int * fds_size_ptr, char * room_name){
  ...
  int err_poll, client_no, fds_size = *fds_size_ptr;
...
void append_to_thread_list(struct thread_data ** thr_data_ptr){

  struct thread_data * thr_data = *thr_data_ptr;
...
void create_menu_thread(){
  ...
  append_to_thread_list(&thr_data);
  ...
  if( pthread_create(&(thr_data->id), NULL, &run_menu_thread, (void *) args) != 0)
    errx(1, "pthread_create");

}


void send_menu_info_to_client(int fd){
...
void process_client_dispatch(char * prefix, char * message, struct pollfd ** fds, int * fds_size, int client_no){
...
static void process_client_request(struct pollfd ** fds, int * fds_size, int client_no){
  ...
  if( err_no == -1) // something went wrong
    ...
    process_client_dispatch(prefix, message, fds, fds_size, client_no);
